...
h_true = plot(x1, x2, '-', 'LineWidth', 1.5);
grid, hold on; 
...
for ii=1:N
    if (y(ii) == 1)
       plot(X(1,ii), X(2,ii), 'bo', 'HandleVisibility','off')
    else
        plot(X(1,ii), X(2,ii), 'r+', 'HandleVisibility','off')
    end
end
...
figure(1)
h_svm = plot(x1, x2, 'm', 'LineWidth', 1.5);
grid on;
xlabel('x_1'), ylabel('x_2'), hold on
legend([h_true h_svm], ...
 { 'random sep. hyperplane $\mathcal{H}_{\mathbf{a},b}$', ...
   'optimal sep. hyperplane $\mathcal{H}_{\mathbf{a}_*,b_*}$' }, ...
...
while (1)                                     % Outer loop 
       ...
       while ( 1 )                            % Inner loop 
          ...
          if (l_x^2/2 <= threshold_1) break; end                             % Newton termination condition
           ...
           str = strcat('Outer Iteration: ',num2str(outer_iter));
           title(str);
       ...
       % Save Figure 2 for current outer iteration
       figure(2);
       filename = fullfile(fig_dir, sprintf('outer_iter_%02d.png', outer_iter));
       saveas(gcf, filename);
